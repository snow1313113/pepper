...
{
    MemLRUSet<TestNode> lru_set;
    bool result = lru_set.init(mem_, mem_size_, max_size_, buckets_num_);
    ASSERT_TRUE(result);

    ASSERT_TRUE(lru_set.empty());
    ASSERT_FALSE(lru_set.full());
    EXPECT_EQ(lru_set.size(), 0ul);
    EXPECT_EQ(lru_set.capacity(), max_size_);
...
TEST(MemLRUSetTest, mem_lru_set_test_0_size)
{
    static const size_t MAX_SIZE = 20;
    static const size_t BUCKETS_NUM = 17;
    size_t mem_size = MemLRUSet<TestNode>::need_mem_size(MAX_SIZE, BUCKETS_NUM);
    ...
    // 在函数调用里面的LRUSet使用，出了这个函数后里面的LRUSet就会调用析构
    zero_size_test(raw_mem.get(), mem_size, MAX_SIZE, BUCKETS_NUM, del_set, node_rand_map);

    // 新的LRUSet对象重新init
    MemLRUSet<TestNode> lru_set;
    bool result = lru_set.init(raw_mem.get(), mem_size, MAX_SIZE, BUCKETS_NUM, true);
    ASSERT_TRUE(result);
