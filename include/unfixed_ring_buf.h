...
public:
    ...
    /// 队列是否空
    bool empty() const { return m_head->m_used_size == 0 && m_head->m_used_size < m_head->m_size; }

    /// 队列是否满了
    bool full() const { return m_head->m_used_size >= m_head->m_size; }

    /// 当前已经使用的字节数
    size_t size() const { return m_head->m_used_size; }

    /// 队列最大字节数容量
    size_t capacity() const { return m_head->m_size; }

    /// 获取插入了多少数据包
    size_t get_num() const { return m_head->m_item_num; }
